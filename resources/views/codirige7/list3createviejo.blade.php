...
<div class="col-sm-7 form-group">
    <div class="input-group">
        <input class="form-control" id="search" value="{{ Session::get('codirige7tesis_search') }}"
               onkeydown="if (event.keyCode == 13) ajaxLoad('codirige7/list3create/{{$PROFESOR_id}}?ok=1&search='+this.value)"
               placeholder="Buscar"
               type="text">

        <div class="input-group-btn">
            <button type="button" class="btn btn-default"
                    onclick="ajaxLoad('codirige7/list3create/{{$PROFESOR_id}}?ok=1&search='+$('#search').val())"><i
...
<th>
            ...
            <a href="javascript:ajaxLoad('codirige7/list3create/{{$PROFESOR_id}}?field=ALU_PATERNO&sort={{Session::get("codirige7tesis_sort")=="asc"?"desc":"asc"}}')">
                Alumno
            </a>
 ...
 if ((2)==($tes->TES_ESTADO))               
  ...
  ?>
           ...
           <a role="button"  class="btn btn-success  btn-circle" 
                   href="javascript:if(confirm('¿Esta seguro que quiere selecionar  esta tesis?')) ajaxLoad('codirige7/create/{{$tes->id}}/{{$PROFESOR_id}}')"><i class="fa fa-check-square-o"></i> 
                     </a>
                ...
                <a class="btn btn-warning btn-circle" title="Show4"
                   href="javascript:ajaxLoad('codirige7/showtesiscreate/{{$tes->id}}/{{$PROFESOR_id}}')">
